...
class Ranker {
    ...
    // cache files
    static public FileInfo CACHE_FILE = new FileInfo(Path.Join(Settings.BASE_DIR.ToString(), "cache.json"));
    static public FileInfo CACHE_WC = new FileInfo(Path.Join(Settings.BASE_DIR.ToString(), "cache_wc.json"));
    static public FileInfo CACHE_VECTORS = new FileInfo(Path.Join(Settings.BASE_DIR.ToString(), "cache_vectors.json"));
    static public FileInfo CACHE_TFIDF = new FileInfo(Path.Join(Settings.BASE_DIR.ToString(), "cache_tfidf.json"));
    ...
    // vectorized document {filename: {word: count}}
    ...
    // {word: [most_relevant_documents]}
    static public TopRanks TopRanked = new TopRanks("tfidf_ranker");

    static public int CACHE_TIMEOUT = 5; // minutes

    public Ranker() {
        // reload cache
        GetFiles(Settings.DATA_DIR);
    }
